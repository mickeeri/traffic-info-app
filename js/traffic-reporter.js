...
TrafficReporter.prototype.renderMessages = function() {
  var srAPI =
    'http://api.sr.se/api/v2/traffic/messages?format=json&pagination=false';
  var that = this;
  ...
  if (that.storedResponse === null) {
    $.getJSON(srAPI, function(data) {
      ...
      $(
        "<div class='api-error'>Trafikmeddelanden kunde inte hämtas</div>",
  ...
  function sortMessages(messages) {
    messages.forEach(function(message) {
      ...
      message.createddate = moment(message.createddate).format(
        'YYYY-MM-DD HH:mm',
      );
    });
  ...
  function renderListItem(message, category, priority) {
    ...
    // Render list
    $(
      "<li class='message-li'><a href='#' class='card-panel message hoverable' id='" +
        message.id +
        "'>" +
        listItem +
        '</a>' +
